...
3. GAS応答ページが、Apps Scriptのsandbox iframeから `window.top.postMessage()` で結果をGitHub Pagesへ返す。
...
hidden iframe内へのGAS応答表示には `HtmlOutput.setXFrameOptionsMode(ALLOWALL)` が必要である。これによりApps Script既定の `X-Frame-Options` は外れるため、GAS側の `parent_origin` 検証、`postMessage()` の固定 `targetOrigin`、サイト側のresponse origin・nonce検証を削除しない。Apps Scriptは外側のGASページ内にsandbox iframeを作るため、messageの `event.source` はform targetの外側iframeと一致せず、source一致検証には使わない。
