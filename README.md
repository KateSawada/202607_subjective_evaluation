...
- `rclone lsjson`からのDrive manifest生成
...
Drive file IDを手作業で調べる必要はない。`rclone lsjson`は各ファイルの `ID`, `Path`, `Size` をJSONとして出力できる。アップロード完了後、rcloneを実行したサーバーで次を行う。

```bash
mkdir -p ~/202607_subjective_evaluation_upload/lsjson
cd ~/202607_subjective_evaluation_upload

rclone lsjson gdrive:202607_subjective_evaluation_audio/seq_flow \
  --files-only --recursive > lsjson/seq_flow.json
rclone lsjson gdrive:202607_subjective_evaluation_audio/joint_ar \
  --files-only --recursive > lsjson/joint_ar.json
rclone lsjson gdrive:202607_subjective_evaluation_audio/txt_ar \
  --files-only --recursive > lsjson/txt_ar.json
```

3つのJSONをこの親repoへ安全に転送した後、repo rootで次を実行する。
...
`manifest`はDrive上のファイル名を変更せず、ローカル生成元basenameとの対応を記録する。`generate`は固定seed `20260713`で手法ごとに独立抽出する。Joint ARが396本未満、Seq. Flow/Txt ARが198本未満の場合は生成を停止し、不足数をエラー表示する。
...
5. `setupSheets()`を1回実行して権限を承認し、`assignments`, `responses`, `submissions`を作る。
6. Web Appを `Execute as: Me`、参加者がアクセス可能な設定でdeployする。
7. deployment URLを `js/config.js` の `gasEndpoint` に設定する。
8. test deploymentで割当、6時間期限、重複送信、iframe応答originを確認する。
