...
- 本番セット数: 66 (`set_001` から `set_066`)
- 1セット: 6 trials
- 全trial数: 396
- 各比較: 198 trials
- Seq. Flow使用数: 198本（200本中2本を未使用）
- Txt AR使用数: 198本（200本中2本を未使用）
- Joint AR使用数: 396本（400本中4本を未使用）
...
暫定仕様として、ファイル名から抽出できる `songNNNN_barNNNN` を生成条件キーとし、比較する2手法で同じキーの音源をペアにする。同じキーが複数ある場合は、ソート順ではなく生成manifest上のサンプルIDで一意に対応付ける。

Joint ARの400本は、198本をTxt AR比較、別の198本をSeq. Flow比較に割り当てる。Joint AR音源を両比較で再利用しない。対応キーが不足・重複する場合は黙ってランダムペアにせず、セット生成を失敗させて診断表を出力する。

このペアリング規則は実装前の要確認事項である。異なるコード条件をランダムに比較する設計を意図する場合は、本節を変更する。

## 3. 画面遷移

- 起動直後に `participant_id` を生成し、刺激リストを取得してGASへセット割当を要求する。
- 割当中は中央に「読み込み中・・・」を表示し、導入画面の開始操作を無効化する。
- 割当成功後、導入画面を表示する。
- 導入画面には実験概要、所要時間、参加環境、匿名化、公表可能性、同意、再読み込み禁止、連絡先を表示する。
- 「次へ」で音楽経験アンケートへ進む。
...
サイトは起動時にGASへ `action=assign_set`, `experiment_id`, `participant_id`, `set_ids` を送る。GASは `LockService.getScriptLock()` の保持中に次を実行する。
...
- set数が66、各setのtrial数が6
- 各setで両比較が3 trialsずつ
- 全体で各比較198 trialsずつ
- 全trialに2設問がある
- Seq. Flow/Txt AR URLが各198個、Joint AR URLが396個で重複しない
...
- 前回の `fetch(..., mode: "no-cors")` はレスポンス本文を確認できず、この要件を満たさないため、第2回では受理確認可能な送信方式へ変更する。
...
- lease期限切れまたはtoken不一致: 回答を保存せず、参加者には実験実施者への連絡を案内
...
1. ペアリングは `songNNNN_barNNNN` が一致する同一生成条件同士でよいか。それとも前回同様、各手法から独立に選んだサンプルを比較するか。
2. セットのlease取得は「ページを開いた直後」でよいか。導入・同意後に取得すると放棄leaseを減らせるが、ユーザー指定に合わせて本仕様では起動直後としている。
3. 6時間経過後に古いページから届いた回答は破棄する方針でよいか。本仕様では一意回答を優先して破棄する。
4. Google Driveの直接URLが公開前ブラウザ試験で不安定だった場合、Google Cloud Storage等への変更を許容するか。
5. GASへの受理確認方式は実装検証が必要である。前回の `no-cors` POSTは使用しない前提でよいか。
