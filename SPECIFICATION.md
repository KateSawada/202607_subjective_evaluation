...
- デスクトップPCまたはノートPCを必須とし、画面幅959px以下では開始不可にする。PCで表示された場合は、ブラウザのウィンドウを広げてページを再読み込みするよう案内する。
...
`no_set_available` を受け取った場合、サイトは「現在利用可能な実験セットがない」ことと「実験実施者へ問い合わせる」ことを参加者へ明示する。
...
`submitted` 状態の一意なセット数が、その時点の総セット数の90%（小数点以下切り上げ）以上に達した時点で、GASはScript Property `NOTIFICATION_EMAIL` に設定された実験実施者へメールを一度だけ送る。未回答または期限切れleaseは数えない。総セット数は割当要求の `set_ids` 件数から取得し、古いページによって減少しないよう最大値を保持する。送信成功後は通知時の総セット数をScript Propertyへ記録し、同じ総数での重複通知を防ぐ。セット追加により総数が増えた場合は、新しい総数に対する90%到達時に再度通知できる。メール送信の失敗は回答保存の成否に影響させず、次の有効回答受理時に再試行する。
...
- 再生・評価画面には内部trial IDの代わりに「比較 N / 6」と表示する。提示音源は楽曲中から取り出した32小節に相当し、楽曲の冒頭・末尾としての自然さや完結性ではなく、提示区間内の音楽的なつながりを評価するよう説明する。
...
5. 応答HTMLはApps Scriptのsandbox iframe内から `window.top.postMessage(result, configuredParentOrigin)` を1回実行する。`targetOrigin` に `*` を使わない。
6. 親ページは `message` イベントについて、許可されたGAS応答origin、`submission_id`、`response_nonce`、データ形式を検証する。Apps Scriptの入れ子sandboxにより `event.source` は外側のform target iframeと一致しないため、source一致は検証条件にしない。
...
GASの `HtmlOutput` はhidden iframe内で読み込めるよう `XFrameOptionsMode.ALLOWALL` を指定する。その代わり、GAS側の許可 `parent_origin` 検証と、親ページ側のmessage origin・nonce検証を必須とする。
