...
`submitted` 状態の一意なセット数が60件以上に達した時点で、GASはScript Property `NOTIFICATION_EMAIL` に設定された実験実施者へメールを一度だけ送る。未回答または期限切れleaseは数えない。送信成功後は通知済み状態をScript Propertyへ記録し、重複通知を防ぐ。メール送信の失敗は回答保存の成否に影響させず、次の有効回答受理時に再試行する。
